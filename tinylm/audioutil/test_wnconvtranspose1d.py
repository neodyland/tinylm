...
from .wnconvtranspose1d import WeightNormConvTranspose1d


class WnConvTranspose1dTest(TorchTinygradCloseTest):
    def __init__(
        self,
        atol: float = 1e-5,
        rtol: float = 1e-5,
        ...
        self.bias = bias
        self.tg_mod = WeightNormConvTranspose1d(
            self.in_channels,
            self.out_channels,
            self.kernel_size,
            stride=self.stride,
        ...
        self.torch_mod = torch.nn.ConvTranspose1d(
            self.in_channels,
            self.out_channels,
            self.kernel_size,
            stride=self.stride,
        ...
        self.tg_mod.weight_g.assign(self.torch_mod.weight_g.detach().numpy())
        if self.bias:
...
def test_wnconvtranspose1d(bs, in_channels, seq_len):
    inputs = np.random.randn(bs, in_channels, seq_len).astype(np.float32)
    test = WnConvTranspose1dTest(
